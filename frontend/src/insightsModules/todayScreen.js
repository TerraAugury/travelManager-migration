...
function twoDayWindow(baseDate) {
  const base = new Date(`${String(baseDate || new Date().toISOString().slice(0, 10))}T00:00:00Z`);
  const tomorrow = new Date(base.getTime() + 86400000);
  return [base.toISOString().slice(0, 10), tomorrow.toISOString().slice(0, 10)];
}
function inWindow(flight, dayA, dayB) {
  const dep = utcDay(flight?.departureScheduled);
  const arr = utcDay(flight?.arrivalScheduled);
  return dep === dayA || dep === dayB || arr === dayA || arr === dayB;
}
function lookupDay(flight, dayA, dayB) {
  const dep = utcDay(flight?.departureScheduled);
  const arr = utcDay(flight?.arrivalScheduled);
  if (dep === dayA || dep === dayB) return dep;
  if (arr === dayA || arr === dayB) return arr;
  return dayA;
}
...
function mergeFlights(primary, secondary) {
  ...
  for (const flight of [...(Array.isArray(primary) ? primary : []), ...(Array.isArray(secondary) ? secondary : [])]) {
    const key = flight?.id ? `id:${flight.id}` : `k:${String(flight?.flightNumber || "").toUpperCase()}|${flight?.departureScheduled || ""}|${flight?.arrivalScheduled || ""}|${flight?.departureCode || ""}|${flight?.arrivalCode || ""}`;
    if (seen.has(key)) continue;
    seen.add(key);
    merged.push(flight);
  }
  return merged.sort((a, b) => String(a?.departureScheduled || "").localeCompare(String(b?.departureScheduled || "")));
}
...
export async function renderTodayScreen({ els, token, api, esc, trips }) {
