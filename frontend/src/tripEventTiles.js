...
function statusBadge(flight) {
  ...
  if (raw.includes("landed") || raw.includes("arriv") || raw.includes("past") || (arr && arr.getTime() < Date.now())) {
...
function layoverCard(prevFlight, nextFlight) {
  ...
  return `<article class="trip-event-card layover-card"><div class="event-card-top"><span class="event-kicker">LAYOVER</span><span class="layover-icon">${PLANE_ICON}</span></div><div class="layover-text">${esc(layover || "Layover")} in ${esc(airport)}</div></article>`;
}
